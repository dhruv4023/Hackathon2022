...
import SubmitedFormList from "./SubmitedFormList/SubmitedFormList";
function ServicePage() {
  const adminUser = useSelector((s) => s.authReducer)?.data;
  const { Sid } = useParams();
  const servN = useSelector((s) => s.serviceReducer)
  ...
  return (
    <div className="container_app1">
      <div className="container_app2">
        ...
        {adminUser && <SubmitedFormList Sid={Sid}servN={servN} />}
      </div>
    </div>
  );
}
